    ...
    props => {
      ...
      return types.map(type => (
        <div key={`checkbox-input-${type}`}>
          <input type="checkbox" id={type} name="interest" value={type} onChange={props.handleChange}></input>
      ...
      return (
        <div>
          <h1>Search + Filter</h1>
          <form>
            <label key="textbox-label-name" htmlFor="name">Name:</label>
            <input key="textbox-input-name" id="name" name="name" type="text" onChange={props.handleChange} />
            <fieldset>
